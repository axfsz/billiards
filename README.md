...
一个可本地双人游玩、也可通过邀请链接进行在线双人对战的产品化网页台球项目。首页提供“快速开始、邀请好友、加入房间”三个明确入口；前端主体在 `billiards.html`，后端使用 Node.js 原生 HTTP 服务和 WebSocket，为在线房间、邀请预览、二维码、回合同步、断线恢复和对局状态持久化提供支持。

## 项目概览

本项目包含两种使用方式：

- 本地模式：首页点击“快速开始”，即可在同一台设备上进行双人台球对局。
- 在线模式：玩家 1 创建房间并分享邀请码、链接或二维码；玩家 2 点击链接后只需确认昵称即可加入，双方通过 WebSocket 同步击球、自由球、重开、僵局重摆和再来一局等事件。
...
- 产品化首页：快速开始、邀请好友和加入房间三个入口，邀请链接会直接进入专用加入流程。
- 完整 Waiting Lobby：显示双方席位、玩法、邀请码、邀请链接、可扫描二维码和等待状态。
- 多渠道邀请：支持复制链接、复制邀请码、Web Share、WhatsApp 和 Telegram。
- 清晰比赛 HUD：显示当前玩家、目标球、球组或比分、剩余球数和在线连接状态，并提供非阻断回合提示。
- 台球物理与规则处理：包含球桌绘制、碰撞、进袋、犯规、自由球、胜负判定等基础逻辑。
- 新手与设置体验：首次三步引导，支持声音、瞄准辅助、振动反馈和中英文切换，设置保存在本地。
- 统一反馈：自由球合法区域、结构化犯规信息、网络恢复遮罩、双方确认弹窗和完整赛果面板。
...
│   ├── product-ux.test.js       # 首页、移动端、i18n 和真实双客户端产品旅程测试
│   └── server.test.js          # 服务端接口、房间状态和 WebSocket 流程测试
├── docs/superpowers/specs/     # 产品体验重构设计说明
├── .dockerignore
└── .gitignore
```
...
只运行 `test/billiards-ui.test.js` 和 `test/product-ux.test.js` 中的浏览器体验测试。
...
| `GET` | `/api/invites/:token` | 获取加入前所需的邀请者昵称、玩法、状态和过期时间 |
| `GET` | `/api/invites/:token/qr.svg` | 生成同源邀请链接二维码 |
...
1. 玩家 1 从首页选择“邀请好友”，确认昵称和玩法后创建在线房间。
2. 服务端生成房间、邀请码、邀请链接、二维码和匿名身份 Cookie。
3. 玩家 2 点击链接后看到邀请者和玩法，确认昵称即可加入；也可以手动输入邀请码或粘贴完整链接。
...
- 服务重启后的房间恢复
- 前端自由球确认和 UI 静态行为
- 三入口首页、无效邀请码反馈、首次新手引导和中英文切换
- 1366×768、1440×900、1920×1080、375×667、390×844、393×852、430×932 布局矩阵
- 两个独立浏览器上下文的邀请链接加入、击球同步、掉线暂停、重连恢复和双方确认重开
- 邀请预览最小字段暴露和同源二维码输出

## 二次开发提示

- 前端仍保持零构建的单文件部署方式，内部已按产品状态、设置/i18n、在线客户端、规则、输入和渲染划分逻辑区域；后续可按风险逐步拆分 ES Modules。
